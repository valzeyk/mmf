...
⭓Anonymous Menu
${simbol} ${prefix}anonymous
${simbol} ${prefix}start
${simbol} ${prefix}next
${simbol} ${prefix}keluar
${simbol} ${prefix}sendkontak
...
case 'bass': case 'blown': case 'deep': case 'earrape': case 'fast': case 'fat': case 'nightcore': case 'reverse': case 'robot': case 'slow': case 'smooth': case 'tupai':
                ...
                zeroyt7.sendMessage(m.chat, { audio: buff, mimetype: 'audio/mpeg' }, { quoted : m })
                fs.unlinkSync(ran)
                })
...
case 'github': case 'gitdl': case 'gitclone': {
                if (!text) throw `Example : ${prefix + command} https://github.com/Sanzykawaiiii`
		...
		let filename = (await fetch(url, {method: 'HEAD'})).headers.get('content-disposition').match(/attachment; filename=(.*)/)[1]
                zeroyt7.sendMedia(m.chat, url, filename, m)
            }
            break
case 'katabijak': {
m.reply(mess.wait)
...
zeroyt7.sendMessage(m.chat, { image: {url: myapi.result }, caption: `Done Nih....`}, { quoted: ftroli })
}
break
case 'xnxxsearch': {
...
case 'anonymous': {
                ...
                zeroyt7.sendButtonText(m.chat, buttons, `\`\`\`Hi Kak Welcome To Anonymous Chat\n\nKlik Button Dibawah Ini Untuk Mencari Partner\`\`\``, zeroyt7.user.name, m)
            }
			break
            case 'keluar': case 'leave': {
                ...
                if (!room) {
                    ...
                    await zeroyt7.sendButtonText(m.chat, buttons, `\`\`\`Kamu Sedang Tidak Berada Di Sesi Anonymous, Tekan Button Untuk Mencari Partner \`\`\``)
                    throw false
                }
                m.reply('Ok')
                let other = room.other(m.sender)
                if (other) await zeroyt7.sendText(other, `\`\`\`Partner Telah Meninggalkan Sesi Anonymous\`\`\``, m)
            ...
            case 'sendkontak': case 'sendcontact': {
                ...
                let profile = await zeroyt7.profilePictureUrl(room.b)
                let status = await zeroyt7.fetchStatus(room.b)
                let msg = await zeroyt7.sendImage(room.a, profile, `Nama : ${await zeroyt7.getName(room.b)}\nBio : ${status.status}\nUser : @${room.b.split("@")[0]}`, m, { mentions: [room.b] })
                zeroyt7.sendContact(room.a, [room.b.split("@")[0]], msg)
            }
            break
            case 'mulai': case 'start': {
                ...
                if (Object.values(this.anonymous).find(room => room.check(m.sender))) {
                    ...
                    await zeroyt7.sendButtonText(m.chat, buttons, `\`\`\`Kamu Masih Berada Di dalam Sesi Anonymous, Tekan Button Dibawah Ini Untuk Menghentikan Sesi Anonymous Anda\`\`\``, zeroyt7.user.name, m)
                    throw false
                }
                ...
                if (room) {
                    ...
                    await zeroyt7.sendButtonText(room.a, buttons, `\`\`\`Berhasil Menemukan Partner, sekarang kamu dapat mengirim pesan\`\`\``, zeroyt7.user.name, m)
                    room.b = m.sender
                    room.state = 'CHATTING'
                    await zeroyt7.sendButtonText(room.b, buttons, `\`\`\`Berhasil Menemukan Partner, sekarang kamu dapat mengirim pesan\`\`\``, zeroyt7.user.name, m)
                } else {
                    let id = + new Date
                    this.anonymous[id] = {
                        id,
                    ...
                    await zeroyt7.sendButtonText(m.chat, buttons, `\`\`\`Mohon Tunggu Sedang Mencari Partner\`\`\``, zeroyt7.user.name, m)
                }
                break
            }
